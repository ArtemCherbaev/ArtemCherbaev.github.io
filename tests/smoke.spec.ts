...
test.describe('every page', () => {
  for (const { path, title, heading } of PAGES) {
    test(`${path} loads, titled and headed, with a clean console`, async ({ page }) => {
...
test('an unknown address answers 404 and offers the way back', async ({ page }) => {
...
test.describe('the landing page', () => {
  ...
  test('the earlier roles are in the document but hidden until asked for', async ({ page }) => {
    const extra = page.locator('.tl-extra');
    await expect(extra.first()).toBeHidden();

    const button = page.getByRole('button', { name: /show the earlier roles/i });
    await expect(button).toHaveAttribute('aria-expanded', 'false');
    await button.click();

    await expect(extra.first()).toBeVisible();
    await expect(page.getByRole('button', { name: /show fewer roles/i })).toHaveAttribute(
      'aria-expanded',
      'true',
    );
  });
  ...
  test('the rest of the skills expand the same way', async ({ page }) => {
    ...
    await expect(extra.first()).toBeHidden();
    await page.getByRole('button', { name: /show the rest of my skills/i }).click();
    await expect(extra.first()).toBeVisible();
  });

  test('the contact form opens on request and takes focus', async ({ page }) => {
  ...
  test('email and GitHub are reachable without opening anything', async ({ page }) => {
    await expect(page.getByRole('link', { name: 'artemcherbaev92@gmail.com' })).toHaveAttribute(
      'href',
      'mailto:artemcherbaev92@gmail.com',
    );
    await expect(page.getByRole('link', { name: 'github.com/artemcherbaev' })).toHaveAttribute(
      'href',
      'https://github.com/artemcherbaev',
    );
  });
});
...
test.describe('accessibility basics', () => {
  for (const { path } of PAGES) {
    test(`${path}: images are described and headings do not skip`, async ({ page }) => {
  ...
  test('the landing page can be reached by keyboard past the header', async ({ page }) => {
    await page.goto('/');
